...
const returnWorld3HQ = document.getElementById('returnWorld3HQ');

if (returnWorld3HQ) {
  returnWorld3HQ.addEventListener('click', function () {
    const world3Content = document.getElementById('world3Content');
    const hq = document.getElementById('arbloxiaHQ');

    if (world3Content) {
      world3Content.hidden = true;
    }
...
function renderWorld3Unlock() {
 ...
 const unlocked = true;
  ...
  if (unlocked) {
    ...
    world3Status.textContent = '🔓 Hiwar Agent';
  } else {
    world3Card.classList.remove('active');
    world3Card.classList.add('locked');

    world3Status.textContent = '🔒 Belum dibuka';
  }
}
renderWorld3Unlock();
window.setInterval(renderWorld3Unlock, 700);

window.ArbloxiaRenderWorld3Unlock = renderWorld3Unlock;
  window.ArbloxiaRenderLevel2 = renderLevel2;
})();
// ========================================
// WORLD 3 - FIDAII YA WATONI
// ========================================
...
(function () {
    ...
    // MASUK WORLD 3
    if (openWorld3Button && world3Content) {
        openWorld3Button.addEventListener('click', function () {

            world3Content.hidden = false;
    ...
    // KEMBALI KE HQ
    if (returnWorld3HQ && world3Content) {
        returnWorld3HQ.addEventListener('click', function () {
